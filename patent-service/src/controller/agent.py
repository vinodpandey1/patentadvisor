...
@tool
def get_queries_of_patent(userId: str, patentID: str, query: str):
    ...
    and use it to get the answer to query."""
    uuid = utils.get_document_uuid(supabase, patentID)
    url = ADVISOR_URL + const.PATENT_QA_URL + userId + "/" + uuid + "?query=" + query
...
@tool
def get_images_of_patent(userId: str, patentID: str, query: str):
...
def get_ai_patent_advisor_agent():
    ...
    tools = [get_queries_of_patent, get_summary_of_patent, get_audio_url_of_patent, get_podcast_url_of_patent, get_images_of_patent]
